...
MyTerm 当前的完成标准是 SSH-first：SSH 连接、终端、SFTP、服务器监控必须可用。笔记、AI、端口转发、Telnet、快捷命令、云同步和 RDP 不纳入当前验收范围；其他已存在入口仅作为保留能力记录。XTerminal 主要作为监控和交互结构参考，不直接引入参考项目源码。
...
| 大文件进度/取消 | ✅ | 本地/远程面板传输显示当前文件字节级进度并支持取消 |
...
| 需求项 | 状态 | 说明 |
| --- | --- | --- |
| 笔记 | 不纳入 | 已有 CRUD、连接过滤、自动保存，但不作为当前验收项 |
| 快捷命令 | 不纳入 | 已有 CRUD、变量展开、一键执行到当前 SSH 终端，但不作为当前验收项 |
| 设置 | ✅ | 本地设置读写 |
| AI 助手 | 不纳入 | 会话和消息可保存；真实模型 API 不作为当前验收项 |
| 端口转发 | 不纳入 | local/dynamic 基础命令存在，关闭会停止监听循环；remote 和流量统计不作为当前验收项 |
| Telnet | 不纳入 | 基础 TCP/Telnet I/O 存在，不作为当前验收项 |
| RDP | 不纳入 | 外部客户端 launcher 存在，不作为当前验收项 |
| 云同步 | 不纳入 | 不作为当前验收项 |
