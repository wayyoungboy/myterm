...
#[tauri::command]
pub fn create_ai_conversation(db: State<'_, DbConn>, title: Option<String>) -> Result<AiConversation, String> {
    let conn = db.0.lock().map_err(|e| e.to_string())?;
    let id = Uuid::new_v4().to_string();
    let title = title.unwrap_or_else(|| format!("Conversation {}", chrono::Local::now().format("%Y-%m-%d %H:%M")));
    conn.execute(
        "INSERT INTO ai_conversations (id, title) VALUES (?1, ?2)",
        rusqlite::params![id, title],
    ).map_err(|e| e.to_string())?;
    Ok(AiConversation { id, title: Some(title), created_at: None })
}
...
#[tauri::command]
pub fn get_ai_messages(db: State<'_, DbConn>, conversation_id: String) -> Result<Vec<AiMessage>, String> {
    let conn = db.0.lock().map_err(|e| e.to_string())?;
    let mut stmt = conn
...
#[tauri::command]
pub fn save_ai_message(db: State<'_, DbConn>, conversation_id: String, role: String, content: String) -> Result<AiMessage, String> {
    let conn = db.0.lock().map_err(|e| e.to_string())?;
    let id = Uuid::new_v4().to_string();
    conn.execute(
    ...
    Ok(AiMessage { id, conversation_id, role, content, created_at: None })
}
